...
function ERC20TTVTBalance(props) {
...
//   const { Moralis } = useMoralis();
  ...
  const getData = async () => {
    ...
    const accounts = await web3.eth.getBalance(TVT_CONTRACT)
    setData(accounts)
}
...
//   }
  ...
  return (
    ...
    display: "flex",
    alignItems: "center",
    justifyContent: "center",}}>{data && data} TVT  <FontAwesomeIcon onClick={() => getData()} style={{height:"45px", marginLeft:"5px", cursor:"pointer"}} icon={faSync} /></div>
    </>
  );
}
