...
const InnerNav = () => {
  ...
  function unHideWord(e, logoTitle, direction) {
    ...
    function putSymbol(counter = 0) {
      if (direction === "right" ? index < 0 : index > logoTitle.length - 1)
        return; //
      if (counter < 2) {
        setTimeout(() => {
    ...
    <div id="nav">
      <div id="nav-inner">
        ...
        <div
          id="nav-left"
          className="noSelect"
          onMouseOver={(e) => !isMobile && unHideWord(e, t("About"), "left")}
        >
          <Link to="about" data-id="About">
            {t("About")}
          </Link>
        </div>
        <div
          id="nav-right"
          className="noSelect"
          onMouseOver={(e) => !isMobile && unHideWord(e, t("Presale"), "right")}
        >
          <Link to="presale" data-id="Buy it">
            {t("Presale")}
          </Link>
        </div>
      </div>
